...
public class ex3_MemoryGame {
    public static void main(String[] args) {
      Scanner scanner = new Scanner(System.in);

      List<String> elements = Arrays.stream((scanner.nextLine()
                .split("\\s+"))).collect(Collectors.toList());
        // first line, you will receive a sequence of elements
      String input;
        // the following lines, you will receive integers until the
        // command "end"
      int moves = 0;
      ...
      while (!"end".equals(input = scanner.nextLine())) {
          ...
          if ((index[0] == index[1]) || (index[0] < 0
                    ...
                    (index[1] < 0 || index[1] >= elements.size())) {
             System.out.println("Invalid input! Adding additional" +
                        " elements to the board");
                elements.add(elements.size() / 2, "-" +
                        moves + "a");
                elements.add(elements.size() / 2, "-" +
                        moves + "a");
          } else {
              if (elements.get(index[0]).equals(elements.get(index[1]))){
                 System.out.printf("Congrats! You have found matching " +
                         "elements - %s!%n", elements.get(index[0]));
